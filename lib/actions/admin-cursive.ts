...
async function upsertResolutionAsVisitor(
  ...
  if (existing) {
    // Segment membership is enrichment evidence, not a site-visit event.
    // Keep lastSeenAt reserved for the real-time pixel/webhook path so
    // attribution windows cannot be refreshed by the polling cron.
