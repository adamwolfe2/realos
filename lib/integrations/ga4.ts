...
// The self-serve /api/oauth/ga4/connect flow (lib/integrations/oauth-handler.ts
...
function authFromDecryptedJson(json: string): GoogleAuth {
  ...
  if ("kind" in parsed && parsed.kind === "oauth") {
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        "GA4: stored credentials are an OAuth token but GOOGLE_OAUTH_CLIENT_ID/GOOGLE_OAUTH_CLIENT_SECRET are not configured — cannot refresh the access token.",
      );
    }
