      ...
      {stages.map((s) => {
        ...
        return (
          <li key={s.stage} className="flex items-center gap-3">
            <span className="w-36 shrink-0 text-xs text-muted-foreground">{s.stage}</span>
    ...
    <ul className="space-y-2">
      {sources.map((row) => (
        <li key={row.source} className="flex items-center gap-3">
          <span className="flex-1 text-sm text-foreground">{row.source}</span>
        ...
        <thead>
          <tr>
            {columns.map((c) => (
              <th
                key={c}
                className="text-left text-[10px] tracking-widest uppercase font-semibold text-muted-foreground pb-2 border-b border-border"
              >
                {c}
              </th>
            ))}
