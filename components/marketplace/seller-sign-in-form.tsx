...
export function SellerSignInForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/marketplace/seller-auth/request", {
        method: "POST",
  ...
  if (sent) {
    return (
      ...
      >
        ...
        >
          We just sent a sign-in link to <span style={{ fontWeight: 600 }}>{email}</span>.
        </p>
        <button
          type="button"
  ...
  return (
    <form className="space-y-3" onSubmit={handleSubmit}>
      ...
      {error && (
        <p style={{ color: "#B91C1C", fontFamily: "var(--font-sans)", fontSize: "13px" }}>
          {error}
        </p>
      )}
    </form>
