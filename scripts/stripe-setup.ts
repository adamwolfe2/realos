...
async function run() {
  ...
  console.log("\n[3/4] Website builds — one-time SKUs");
  for (const build of WEBSITE_BUILDS) {
    console.log(`\n  Build: ${build.productName}`);
    const product = await findOrCreateProduct({
      lookupKey: build.productLookupKey,
      name: build.productName,
      description: build.productDescription,
    });
    // Website builds are one-time, no recurring block. The price
    // shape mirrors a simple flat one-off charge.
    const price = await findOrCreatePrice({
      lookupKey: build.priceLookupKey,
      productId: product.id,
      unitAmountCents: build.unitAmountCents,
      metadata: {
        kind: "website_build",
        build_id: build.id,
      },
    });
    priceIdMap[build.priceLookupKey] = price.id;
  }

  console.log("\n[4/4] Writing price-ids.generated.ts");
