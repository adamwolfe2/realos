...
export default async function AttributionPage({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Filter bar — date range + per-property dropdown. Form-driven so
          the URL stays bookmarkable and the page is fully SSR. */}
      <form
        action="/portal/attribution"
        className="rounded-lg border border-border bg-card p-3 grid grid-cols-1 md:grid-cols-[1fr,1fr,1fr,auto] gap-3 items-end"
      >
        ...
        <button
          type="submit"
          className="inline-flex items-center justify-center rounded-md bg-foreground text-background px-4 py-2 text-xs font-semibold hover:opacity-90"
        >
          Apply
        </button>
      </form>
      ...
      {/* Row 4: city split */}
      <section className="grid grid-cols-1 lg:grid-cols-[1fr,1fr] gap-4">
        <SourceDonut
      ...
      <section className="rounded-lg border border-dashed border-border bg-muted/20 px-5 py-4">
        <p className="text-[10px] tracking-widest uppercase font-semibold text-muted-foreground">
          What you get with LeaseStack that Clarity can&apos;t deliver
        </p>
        <ul className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-foreground">
          <li>
            <Link
