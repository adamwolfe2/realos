export default function ChatbotLoading() {
  return (
    <div className="space-y-6 animate-pulse">
      <div className="space-y-1">
        <div className="h-7 w-32 bg-muted rounded-[2px]" />
        <div className="h-4 w-72 bg-muted/60 rounded-[2px]" />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="ls-card p-5 space-y-5">
          <div className="h-4 w-28 bg-muted rounded-[2px]" />
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="space-y-1.5">
              <div className="h-3 w-24 bg-muted/60 rounded-[2px]" />
              <div className="h-9 w-full bg-muted/50 rounded-[2px]" />
            </div>
          ))}
          <div className="space-y-1.5">
            <div className="h-3 w-28 bg-muted/60 rounded-[2px]" />
            <div className="h-20 w-full bg-muted/50 rounded-[2px]" />
          </div>
          <div className="h-9 w-28 bg-muted rounded-[2px]" />
        </div>
        <div className="ls-card p-5 space-y-3">
          <div className="h-4 w-20 bg-muted rounded-[2px]" />
          <div className="rounded-[2px] border border-border bg-muted/20 h-72 flex flex-col justify-end p-3 space-y-2">
            <div className="h-8 w-3/4 bg-muted/50 rounded-[2px] self-start" />
            <div className="h-8 w-1/2 bg-secondary rounded-[2px] self-end" />
            <div className="h-8 w-2/3 bg-muted/50 rounded-[2px] self-start" />
          </div>
          <div className="h-9 w-full bg-secondary rounded-[2px]" />
        </div>
      </div>
    </div>
  );
}
