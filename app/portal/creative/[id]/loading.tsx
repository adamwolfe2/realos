export default function CreativeRequestDetailLoading() {
  return (
    <div className="space-y-6 max-w-3xl animate-pulse">
      <div className="h-4 w-24 bg-muted/60 rounded-[2px]" />
      <div className="space-y-2">
        <div className="h-7 w-72 bg-muted rounded-[2px]" />
        <div className="h-4 w-48 bg-muted/60 rounded-[2px]" />
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="ls-card p-4 space-y-2">
            <div className="h-3 w-20 bg-muted/60 rounded-[2px]" />
            <div className="h-5 w-24 bg-muted rounded-[2px]" />
          </div>
        ))}
      </div>
      <div className="ls-card p-5 space-y-4">
        <div className="h-5 w-32 bg-muted rounded-[2px]" />
        <div className="h-4 w-full bg-secondary rounded-[2px]" />
        <div className="h-4 w-3/4 bg-secondary rounded-[2px]" />
        <div className="h-4 w-5/6 bg-secondary rounded-[2px]" />
      </div>
      <div className="ls-card p-5 space-y-3">
        <div className="h-5 w-24 bg-muted rounded-[2px]" />
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="flex gap-3 py-2">
            <div className="h-4 w-4 rounded-full bg-muted shrink-0" />
            <div className="flex-1 space-y-1">
              <div className="h-4 w-48 bg-muted rounded-[2px]" />
              <div className="h-3 w-32 bg-muted/50 rounded-[2px]" />
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
