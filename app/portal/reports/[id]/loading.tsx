export default function ReportDetailLoading() {
  return (
    <div className="space-y-8 max-w-3xl animate-pulse">
      <div className="h-4 w-24 bg-muted/60 rounded-[2px]" />
      <div className="space-y-2">
        <div className="h-8 w-64 bg-muted rounded-[2px]" />
        <div className="h-4 w-48 bg-muted/60 rounded-[2px]" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="ls-card p-4 space-y-2">
            <div className="h-3 w-20 bg-muted/60 rounded-[2px]" />
            <div className="h-7 w-14 bg-muted rounded-[2px]" />
          </div>
        ))}
      </div>
      <div className="ls-card p-5 space-y-3">
        <div className="h-5 w-32 bg-muted rounded-[2px]" />
        <div className="h-24 w-full bg-secondary rounded-[2px]" />
      </div>
      <div className="ls-card p-5 space-y-3">
        <div className="h-5 w-40 bg-muted rounded-[2px]" />
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-4 w-full bg-secondary rounded-[2px]" />
        ))}
      </div>
    </div>
  );
}
