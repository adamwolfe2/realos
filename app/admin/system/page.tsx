...
const CRON_SCHEDULE: Array<{ jobName: string; schedule: string; description: string }> = [
  ...
  { jobName: "pixel-segment-sync", schedule: "*/5 * * * *", description: "Every 5 min — pull resolved visitors from AudienceLab segment (Cursive pixel self-heal)" },
  { jobName: "reputation-scan", schedule: "0 8 * * *", description: "Daily — per-property reputation refresh (Reddit + Yelp + Google + Tavily)" },
  { jobName: "dataforseo-sync", schedule: "0 5 * * *", description: "Daily — DataForSEO ranked keywords + backlinks" },
